...
function pre_fill() {
    ...
    [0,1,2].forEach((i)=>{
        var canva = document.getElementById("canva"+i);
    ...
    swiper = new Swiper(".mySwiper", {
      ...
      swiper.on('slideChange',function(){
        ...
        setTimeout(()=>{
            ...
            if(dir == 0){
                fill_kural(cur_kural-2)
            }
            else if(dir == 2){
                fill_kural(cur_kural)
            }
...
function fill_kural_i(n, i) {
    ...
    share_cnt_span.style.opacity = 0;
    show_share_cnt();
...
function search_input() {
    ...
    if(cur_chap == -1)
        ...
        tk_search = tk.kural.slice(cur_chap*10, (cur_chap+1)*10);

    tk_search.forEach((e, i) => {
        if (e.line1.indexOf(search) > -1 || e.line2.indexOf(search) > -1 || e.translit.toLowerCase().indexOf(search) > -1) {
            ...
            kural_filter_i.push(cur_chap==-1?i:cur_chap*10+i);
        }
    })
...
function cur_chap_txt_click(){
    chap_txt.value = "";
    search_chap();
    setTimeout(()=>{chap_txt.focus();console.log(1)},200);

}

function search_chap() {

    var search = chap_txt.value.toLowerCase();

    var inn_htm = `
    <li class="dropdown-item" onclick="set_chap(-1)">               
        <div>அனைத்து அதிகாரங்கள்</div> 
        <div>ALL Chapters</div>
        <hr>
    </li>
    `;

    for(var i = 0; i<chap_tam.length; i++){
        if(!search || (search && (chap_eng[i] + chap_tam[i] + chap_eng_translit[i] ).toLowerCase().indexOf(search)>-1 )){
            inn_htm += `
            <li class="dropdown-item" onclick="set_chap(${i})">               
                <div>${chap_tam[i]}</div> 
                <div>${chap_eng[i]}</div>
                <hr>
            </li>`;
...
async function show_share_cnt() {
    ...
    if (cnt > 0) {
        ...
        share_cnt_span.style.opacity = 1;
    }
